...
uint256 hashGenesisBlock("0x0000022011f0e64f9a32b8c344ef28df2bbf30183867060a5ebde875dc5b159f");
...
const string strMessageMagic = "MagCoin Signed Message:\n";
...
double GetLocDifficulty(const CBlockIndex* blockindex)
{
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy = 20 * diff * COIN;
    nSubsidy = nSubsidy / (97200 + nHeight);
    nSubsidy = nSubsidy * 97200;
    nSubsidy >>= (nHeight / 30000);
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 5 * 60 * 60; // MagCoin: 5 hours [deprecated retarget]
static const int64 nTargetSpacing = 180; // MagCoin: 3 minute blocks
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlock *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    /* current difficulty formula, MagCoin - kimoto gravity well */
...
{
    static const int64 BlocksTargetSpacing = nTargetSpacing; //sets based on nTargetSpacing
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal magcoin miner
...
bool CBlock::AcceptBlock()
{
    ...
    // Reject block.nVersion=1 blocks when 85% (75% on testnet) of the network has upgraded:
        if (nVersion < 2)
        {
            if ((!fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 850, 1000)) ||
                (fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 75, 100)))
            {
                return error("AcceptBlock() : rejected old nVersion[1] block");
            }
        }
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MagCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000a65cd6f26d8c623ecdc04cdbd081153a32b2169994e8e0bc65a7284c23d");
    }
    ...
    if (mapBlockIndex.empty())
    {
        if (!fAllowNew)
            return false;
        // Genesis block
        const char* pszTimestamp = "American Ebola Patient Begins Recovery";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
	block.nTime    = 1407522600;
              //block.nTime    = 1407270000; || MainTest Network
        block.nBits    = 0x1e0ffff0;
	block.nNonce   = 1022937;
              //block.nNonce   = 23466; || MainTest Network

        if (fTestNet)
        {
            block.nTime    = 1407270000;
            block.nNonce   = 23466;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x6c738a0a1ad25cfa294990b9558138af8bc7932554932be5e3adac949337865e"));
...
unsigned char pchMessageStart[4] = { 0xd3, 0xb7, 0xc0, 0xf4 };
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    usleep(10 * 1000); //sleep 10ms to get val
